...
def data_tab(request):
    ...
    df = pd.read_excel("C:\\Users\\lenovo\\Desktop\\CRM_cleaned_final.xlsx")
...
def descriptive_statistics_tab(request):
    # Read Titanic dataset from CSV
    df1 = pd.read_excel("C:\\Users\\lenovo\\Desktop\\CRM_cleaned_final.xlsx")
    df = pd.read_excel("C:\\Users\\lenovo\\Desktop\\CRM_cleaned.xlsx")
    # Perform descriptive statistics using pandas
    descriptive_stats = df1.describe().to_html(classes='table table-bordered table-hover')
...
def exploratory_data_analysis_tab(request):
    ...
    default_x_feature = 'Quantity'
    default_y_feature = 'Profit'
    ...
    fig_scatter = px.scatter(df, x=selected_x_feature, y=selected_y_feature, color='Profit',
